...
#show: template.with(title: "图文并排布局")
...
#tufted.margin-note[
  💡 *提示* \
  `side-by-side` 函数位于 `tufted-lib/layout.typ` 模块中。
]
...
#warning-box[
  使用时请注意以下几点，以确保最佳显示效果。
]

+ *图片路径* - 使用相对路径（相对于当前 `.typ` 文件）
+ *ratio 参数* - 只能使用预设值：`"small"`, `"medium"`, `"large"`, `"half"`
+ *文字内容* - 建议用 `[]` 包裹，支持完整的 Typst 语法
